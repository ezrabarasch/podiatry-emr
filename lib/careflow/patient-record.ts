 ...
 * diagnosis already on the chart gets its lastConfirmedAt/lastAssertedByVisitId
...
export async function reconcilePatientMedications(
  ...
  for (const med of medications) {
    ...
    // NOTE: (name, dosage, frequency) uniqueness can't use upsert here, because
    // dosage/frequency are nullable and Postgres unique indexes never treat NULL
    // as equal to NULL - so upsert on the compound key can't dedupe null-valued
    // meds and would create duplicate rows. Find-then-write handles nulls
    // correctly (explicit IS NULL match via Prisma's { equals: null }).
