...
export default function VisitsPage() {
  ...
  // Filtering happens server-side so it applies across pages, not just this one.
  ...
  return (
    <PageShell>
      ...
      <div className="flex flex-wrap items-center gap-3 mb-4">
        <select value={status} onChange={e => onFilter(setStatus)(e.target.value)} className={inputCls}>
          <option value="">All statuses</option>
          <option value="draft">Draft</option>
          <option value="signed">Signed</option>
        </select>
        <select value={providerId} onChange={e => onFilter(setProviderId)(e.target.value)} className={inputCls}>
        ...
        <label className="flex items-center gap-2 text-sm text-text-muted">
          From <input type="date" value={from} onChange={e => onFilter(setFrom)(e.target.value)} className={inputCls} />
        </label>
        <label className="flex items-center gap-2 text-sm text-text-muted">
          To <input type="date" value={to} onChange={e => onFilter(setTo)(e.target.value)} className={inputCls} />
        </label>
      </div>
      ...
      {paged.loading ? (
        ...
        <>
          <Table columns={columns} rows={paged.rows} onRowClick={v => router.push(`/visits/${v.id}`)} empty="No visits found." />
          <Pagination {...paged} onPageChange={paged.setPage} onLimitChange={paged.setLimit} unit="visits" />
        </>
      )}
    </PageShell>
  )
